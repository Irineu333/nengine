...
| `add-lua-scripting` | Segunda implementação do `ScriptHost` SPI via LuaJ 3.0.x. Adiciona `:engine-bundle-lua` com `LuaScriptHost`, tabela global `nengine.*`, `require` sandboxado, signals e reflexão; migra `:games:tictactoe` de Python para Lua, virando sentinela do segundo backend de render **e** do segundo backend de scripting. |
