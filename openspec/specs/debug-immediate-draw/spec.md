# debug-immediate-draw Specification

## Purpose

Facade de desenho imediato (immediate-mode) para debug: `tree.debug.draw` expõe duas superfícies `DebugCanvas` — `world` (sob a view transform da `Camera2D`) e `screen` (em pixels de tela) — com verbos espelhando o `Renderer`. Comandos são single-frame: enfileirados durante `process`/`physicsProcess`, emitidos no render daquele tick e descartados. Verbos viram no-op quando o desenho está desabilitado, e a facade é acessível de scripts Python e Lua.
