...
- `6` Tumbling swarm — 16 quadrados `CharacterBody2D` (24×24) com velocidade linear **e** angular, colidindo contra paredes e entre si por **impulso normal elástico + fricção tangencial de Coulomb**: `jn = -2·(v_rel·n) / denom` (e=1) e `jt = min(velTang/denomT, MU·|jn|)` aplicados juntos no ponto de contato. O impulso muda velocidade linear E angular simultaneamente — bater na quina gera spin (lever arm tangencial), bater face-a-face só inverte direção (lever arm paralelo a `n`), e a fricção acopla sliding ↔ spin: quadrado raspando numa parede converte parte da velocidade tangencial em rolagem, e dois quadrados se atritam ao encostarem. O sweep entra pelo caminho `sweepRotatedRectRotatedRect` porque cada quadrado tem `transform.rotation != 0f`. F2 mostra os AABBs envelope dos OBBs. O ponto de contato é aproximado pelo **canto líder** do OBB rotacionado na direção `-n` (empates dentro de epsilon viram midpoint da face → zero spin em face-a-face; canto isolado em hit oblíquo → componente tangencial em `rA` → spin induzido). `collision.point` ainda devolve centro do OBB; refinar `SweepResult.point` para o ponto geométrico real é trabalho futuro.
- `F1` liga/desliga overlay de FPS (tratado pelo `GameHost`, configurável via `GameConfig.toggleFpsKey`)
- `F2` liga/desliga visualização de colliders (idem, via `GameConfig.toggleCollidersKey`)
