...
- [ ] 1.1 Add `MouseButton` enum (`Left`, `Right`, `Middle`) under `com.neoutils.engine.input`
...
- [ ] 2.5 Update `GameSurface` `pointerInput` block to detect button press/release events and forward them to `ComposeInput`
- [ ] 2.6 Implement `ComposeRenderer.drawLine` on top of `DrawScope.drawLine`

## 3. Tic-tac-toe module scaffolding

- [ ] 3.1 Add `games/tictactoe/` directory with `build.gradle.kts` (depends on `:engine` and `:engine-compose`, applies Kotlin JVM + Compose Desktop application plugins)
