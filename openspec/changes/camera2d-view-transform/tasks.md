...
- [x] 8.4 Validar manualmente em janela 800×600 + janela redimensionada (ex.: 1280×900 + ultrawide) que: paddle alcança topo e base do play field, centerLine sempre vertical e centrada, ball sempre quica nas paredes do mundo virtual, letterbox bars aparecem como esperado.
...
- [x] 12.1 Rodar `./gradlew :games:pong:run` em janela 800×600 e em janela redimensionada (1280×900 e ultrawide). Confirmar: paddle alcança topo/base; centerLine sempre vertical centrada; ball nunca escapa do mundo virtual; F1 mostra FPS; F2 mostra collider bounds projetados corretamente sobre o mundo.
- [x] 12.2 Rodar `./gradlew :games:demos:run` percorrendo 1–5 em janela 800×600 e redimensionada. Confirmar: orbit/scale centralizados; spawner adiciona/remove ok; stress sem regressão; rotating box ok; F2 mostra AABBs projetados.
- [x] 12.3 Rodar `./gradlew :games:tictactoe:run` em janela 600×600 e redimensionada. Confirmar: zero mudança visual; clique acerta a célula correta; vitória/empate/restart ok.
- [x] 12.4 Rodar `./gradlew test` em `:engine`, `:engine-skiko`, `:engine-compose` — todos verdes.
- [x] 12.5 Rodar `openspec verify camera2d-view-transform` (via `/opsx:verify`) e endereçar quaisquer descobertas antes do archive.
