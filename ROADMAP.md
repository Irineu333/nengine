...
| `debug-physics-gizmos` | Expõe o que o `PhysicsSystem` calcula mas esconde: forma real do collider (círculo, retângulo rotacionado) em vez de só AABB, pontos de contato, normais de colisão, vetores de velocidade e sweep do TOI. Maior retorno didático; consome a primitiva de `debug-immediate-draw`. |
