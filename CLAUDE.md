...
- `6` Tumbling swarm — 16 quadrados `CharacterBody2D` (24×24) com velocidade linear **e** angular, colidindo contra paredes e entre si por **impulso rotacional**: `jn = -(1+e)·(v_rel·n) / denomN` (e=1). Wall hits também aplicam impulso tangencial de Coulomb capped em `MU·|jn|` — quadrado raspando converte sliding em rolling. Pair hits ficam puros bilhar-elástico (sem fricção inter-corpos, que tendia a "gerar spin do nada"). Ponto de contato: para wall, o **canto líder** do OBB na direção `-n`; para pair, o **midpoint** dos cantos líderes de A e B na direção um do outro (`rA ≈ -rB`, mantendo as alavancas balanceadas e a transferência de momento angular simétrica). Bater na quina gera spin (lever arm tangencial), bater face-a-face só inverte direção (lever arm paralelo a `n`). O sweep entra pelo caminho `sweepRotatedRectRotatedRect` porque cada quadrado tem `transform.rotation != 0f`. F2 mostra os AABBs envelope dos OBBs. `collision.point` ainda devolve centro do OBB; refinar `SweepResult.point` para o ponto geométrico real é trabalho futuro.
- `F1` liga/desliga overlay de FPS (tratado pelo `GameHost`, configurável via `GameConfig.toggleFpsKey`)
- `F2` liga/desliga visualização de colliders (idem, via `GameConfig.toggleCollidersKey`)
