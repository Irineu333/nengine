...
- `6` Tumbling swarm — 16 quadrados `CharacterBody2D` (24×24) com velocidade linear **e** angular, colidindo elasticamente contra paredes e entre si por **resposta de impulso rotacional**: `j = -2·(v_rel·n) / denom` onde `denom` inclui `(r×n)²/I` em ambos os lados. Aplicar `j·n` no ponto de contato muda velocidade linear E angular ao mesmo tempo — bater na quina gera spin, bater face-a-face só inverte direção, e uma colisão glancing entre dois quadrados transfere momento angular de um para outro conservando momento + energia. O sweep entra pelo caminho `sweepRotatedRectRotatedRect` porque cada quadrado tem `transform.rotation != 0f`. F2 mostra os AABBs envelope dos OBBs. O ponto de contato é aproximado pelo **canto líder** do OBB rotacionado na direção `-n` (empates dentro de epsilon viram midpoint da face → zero spin em face-a-face; canto isolado em hit oblíquo → componente tangencial em `rA` → spin induzido). `collision.point` ainda devolve centro do OBB; refinar `SweepResult.point` para o ponto geométrico real é trabalho futuro.
- `F1` liga/desliga overlay de FPS (tratado pelo `GameHost`, configurável via `GameConfig.toggleFpsKey`)
- `F2` liga/desliga visualização de colliders (idem, via `GameConfig.toggleCollidersKey`)
